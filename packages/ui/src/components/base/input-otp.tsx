...
>(({ index, className, ...props }, ref) => {
  ...
  const values = inputOTPContext.slots[index];
  if (!values) {
    throw new Error("InputOTPSlot must be a child of InputOTP");
  }
  const { char, hasFakeCaret, isActive } = values;
...
>(({ ...props }, ref) => <Minus ref={ref} role="separator" {...props} />);
